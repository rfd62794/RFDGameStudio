      ...
      <div className="w-full flex justify-between items-center mb-3">
        ...
        <div className="flex items-center gap-2 text-xs font-mono">
          <div className="flex items-center gap-1">
            <span className="px-1.5 py-0.5 bg-[#C4C3C0] text-[#141414] border border-[#141414] text-[9px] font-bold uppercase">Fog of War</span>
          </div>
          <div className="flex items-center gap-1">
            <span className="px-1.5 py-0.5 bg-cyan-300 text-[#141414] border border-[#141414] text-[9px] font-bold uppercase">Vanguard</span>
          </div>
          <div className="flex items-center gap-1">
            <span className="px-1.5 py-0.5 bg-amber-300 text-[#141414] border border-[#141414] text-[9px] font-bold uppercase">Rivals</span>
          </div>
        </div>
      </div>
              ...
              <g key={cell.id}>
                ...
                {isScouted && (
                  <g className="pointer-events-none" style={{ opacity: isHovered || isSelected ? 1 : 0.9 }}>
                    ...
                    {/* Show Unit Count Indicator inside the cell (ONLY visible if cell is fully Visible to player,
                        or if it belongs to the player) */}
                    {(isVisible || cell.ownerId === playerCorpId) && (
                      <g transform={`translate(${cell.seed.x + 10}, ${cell.seed.y + 10})`}>
                        {/* Combined unit count bubble */}
                        {(() => {
                           const count = cell.units.circle + cell.units.square + cell.units.triangle;
                    ...
                    {/* If cell is contested, draw a battle icon */}
                    {contested && (
                      <g transform={`translate(${cell.seed.x}, ${cell.seed.y - 12})`}>
                        <circle cx="0" cy="0" r="8" fill="#ef4444" stroke="#141414" strokeWidth="1.5" />
                        <text x="0" y="3" fontSize="8" textAnchor="middle" fill="white" fontWeight="bold">⚔️</text>
                      </g>
                    )}
