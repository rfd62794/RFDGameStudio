 ...
 * Generates procedural retro/tactical sound effects without external audio files.
 */
...
class SoundEngine {
  ...
  // 1. Scrap Pickup Chime
  public playScrapPickup() {
  ...
  public playBreachDetonation() {
    this.playExplosion();
  }

  // 3. Breaching Charge Planted / Beep
  public playBeep() {
    if (this.isMuted) return;
    this.initContext();
    if (!this.ctx) return;
  ...
  // 6. ADR 004: Reality Collapse Emergency Siren
  public playRealityCollapseAlarm() {
    if (this.isMuted) return;
    this.initContext();
    if (!this.ctx) return;
    ...
    [0, 0.22, 0.44].forEach((offset) => {
      ...
      osc.frequency.exponentialRampToValueAtTime(330, t + offset + 0.18);
  ...
  // 7. Extraction Countdown / Success
  ...
  // 9. Resonance Spark Pickup (Crystalline Harmonic Overclock Chime)
  public playOverclockSpark() {
    if (this.isMuted) return;
    this.initContext();
    if (!this.ctx) return;
  ...
  // 10. Reality Klaxon Alarm (3:00 Reality Collapse & Apex Deployment)
  public playRealityKlaxon() {
    if (this.isMuted) return;
    this.initContext();
    if (!this.ctx) return;
    ...
    for (let i = 0; i < 2; i++) {
      ...
      osc.frequency.linearRampToValueAtTime(110, startTime + 0.3);
  ...
  // 11. Biological Squelch / Chitin Break
  public playCrawlerHit() {
    if (this.isMuted) return;
    this.initContext();
    if (!this.ctx) return;
  ...
  // 12. ADR 007: Kinetic Scattergun Fire (Heavy pneumatic mechanical punch)
  public playKineticFire() {
    if (this.isMuted) return;
    this.initContext();
    if (!this.ctx) return;
  ...
  // 13. ADR 007: Plasma Pulse Array Fire (Sub-space ionizing chirp)
  public playPlasmaFire() {
    ...
    osc.frequency.exponentialRampToValueAtTime(220, this.ctx.currentTime + 0.14);
  ...
  // 14. ADR 007: Bullet Wall Ricochet / Impact
  public playRicochet() {
    if (this.isMuted) return;
    this.initContext();
    if (!this.ctx) return;
  ...
  // 15. ADR 008: Hostile Ranged Fire (Synthesized hostile laser discharge)
  public playHostileFire() {
    if (this.isMuted) return;
    this.initContext();
    if (!this.ctx) return;
  ...
  // 16. ADR 011: Tactical Construction & Upgrade Chime
  ...
  // 18. ADR 011: Dry Fire / Resource Error Buzzer
  public playDryFire() {
    if (this.isMuted) return;
    this.initContext();
    if (!this.ctx) return;
