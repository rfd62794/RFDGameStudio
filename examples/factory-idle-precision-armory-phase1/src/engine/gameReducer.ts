    ...
    case 'TICK': {
      ...
      for (const item of currentItems) {
        ...
        if (tile.type === 'seller' && tile.isEnabled !== false) {
          ...
          if (item.kind === 'weapon') {
            ...
            saleVal = Math.round(recipe.salePrice * 1.1); // 10% direct export bonus
          ...
          logs.unshift({
            ...
            text: `Exported ${item.itemId.toUpperCase()} via Port (+${saleVal}$)`,
            type: 'sale',
            tick,
          });
          continue;
          ...
          if (item.kind === 'weapon') {
            ...
            if ((shelfStock[wId] || 0) < state.shelfCapacity) {
              ...
              logs.unshift({
                ...
                text: `Packed ${WEAPON_RECIPES[wId].name} onto retail shelf.`,
                type: 'craft',
                tick,
              });
              continue;
        ...
        // Belt Splitter (Round-Robin alternation)
        if (tile.type === 'splitter' && tile.isEnabled !== false) {
          const splitState = tile.splitterState || 0;
          if (splitState === 0) {
            outDir = tile.direction; // Forward
            tile.splitterState = 1;
          } else if (splitState === 1) {
            outDir = turnRight(tile.direction); // Right
            tile.splitterState = 2;
          } else {
            outDir = turnLeft(tile.direction); // Left
            tile.splitterState = 0;
          }
        }
      ...
      for (const cust of state.activeCustomers) {
        ...
        if (shelfStock[cust.weaponId] >= cust.quantity) {
          ...
          logs.unshift({
            ...
            text: `FULFILLED: ${cust.customerName} bought ${cust.quantity}x ${recipe.name} (+$${revenue})`,
            type: 'sale',
            tick,
          });
        ...
        if (remainingPatience <= 0) {
          ...
          logs.unshift({
            ...
            text: `ORDER EXPIRED: ${cust.customerName} walked out unsatisfied (-5% Rep)`,
            type: 'miss',
            tick,
          });
      ...
      if (nextCustomerSpawn <= 0) {
        if (activeCustomers.length < 4) {
          ...
          const names = ['Captain Vargas', 'Agent Vance', 'Sheriff Thorne', 'Commander Hayes', 'Operative Cruz', 'Marshal Stone'];
          const roles = ['Tactical SWAT Unit', 'Federal Task Force', 'County Constabulary', 'Private Security Detail', 'Special Recon Group'];
          ...
          activeCustomers.push({
            ...
            quantity: 1,
            maxPatienceTicks: 140,
            remainingPatienceTicks: 140,
            bonusMultiplier: 1.0,
            avatarBg: '#0f766e',
          });
        }
        nextCustomerSpawn = 50 + Math.floor(Math.random() * 30);
      }
