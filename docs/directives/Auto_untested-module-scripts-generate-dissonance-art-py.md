# scripts/generate_dissonance_art.py has no tests while its neighbours do
...
```
scripts/generate_dissonance_art.py
```
...
`scripts/generate_dissonance_art.py` has no tests, while other modules in the same folder do. Read it, then write
