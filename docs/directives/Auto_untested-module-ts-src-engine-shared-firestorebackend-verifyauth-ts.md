# ts/src/engine/shared/firestoreBackend/verifyAuth.ts has no tests while its neighbours do
...
```
ts/src/engine/shared/firestoreBackend/verifyAuth.ts
```
...
`ts/src/engine/shared/firestoreBackend/verifyAuth.ts` has no tests, while other modules in the same folder do. Read it, then write
...
| Base branch | - |
| Base commit | ee55f8bd7fdf43ff7c4074d9d19d1b8479aa6c5f |
| Policy | untested-module |

**Status log**
- 2026-09-22 16:15 · backlog-policy · none → Queued — generated from a untested-module finding authorised in backlog_policy.yaml
