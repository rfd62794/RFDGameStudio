# ts/src/engine/shared/firestoreBackend/firebaseAdmin.ts has no tests while its neighbours do
...
```
ts/src/engine/shared/firestoreBackend/firebaseAdmin.ts
```
...
`ts/src/engine/shared/firestoreBackend/firebaseAdmin.ts` has no tests, while other modules in the same folder do. Read it, then write
...
| Base branch | - |
| Base commit | 2d584fbf49a5c94626eaf7896c49bbd11c8006f9 |
| Policy | untested-module |

**Status log**
- 2026-09-22 15:44 · backlog-policy · none → Queued — generated from a untested-module finding authorised in backlog_policy.yaml
