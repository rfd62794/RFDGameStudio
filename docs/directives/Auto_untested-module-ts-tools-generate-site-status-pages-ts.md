# ts/tools/generate-site-status-pages.ts has no tests while its neighbours do
...
```
ts/tools/generate-site-status-pages.ts
```
...
`ts/tools/generate-site-status-pages.ts` has no tests, while other modules in the same folder do. Read it, then write
...
- 2026-09-23 12:05 · robert-claude · Review → Done
<!-- queue:end -->
