...
## Interactive Status Board — Site Pages (Hugo) — COMPLETED
...
3. **Tool script**: `ts/tools/generate-site-status-pages.ts` → writes
   Hugo markdown to `docs/site-status-pages/` (staging dir)
4. **Sync script**: `RFD_IT_Services_Site/scripts/site/sync_status_pages.py`
   → ports staging files to `content/projects/`

### Real new page locations

**In RFDGameStudio (source):**
- `ts/src/status/site-pages.types.ts` — type definitions
- `ts/src/status/site-pages.data.ts` — structured data (5 projects + hub)
- `ts/src/status/generateSitePages.ts` — pure generator function
- `ts/tools/generate-site-status-pages.ts` — generation tool script
- `docs/site-status-pages/` — staging directory (6 generated .md files)
- `ts/tests/test_site_status_pages.ts` — 17 test anchors (5 describe blocks)

**In RFD_IT_Services_Site (ported via sync):**
- `content/projects/studio-status.md` — hub page
- `content/projects/studio-status-shoal.md` — Shoal detail
- `content/projects/studio-status-planet-of-greed.md` — Planet of Greed detail
- `content/projects/studio-status-mutant-battle-ball.md` — Mutant Battle Ball detail
- `content/projects/studio-status-facility-escape.md` — Facility Escape detail
- `content/projects/studio-status-infrastructure.md` — Studio Infrastructure detail
- `scripts/site/sync_status_pages.py` — sync script (ports staging → content)
...
- [x] Dedicated breakdown pages use the real, full content from §1
