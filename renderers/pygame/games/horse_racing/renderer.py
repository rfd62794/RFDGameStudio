...
class HorseRacingRenderer(PyGameEngine):
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        # UI_BUTTON_PRESSED is an output event generated by pygame_gui after
        # processing mouse clicks — process_events() returns False for it,
        # so check for it BEFORE the handle_event gate, not inside it.
        if event.type == pygame_gui.UI_BUTTON_PRESSED:
            self._handle_button_event(event.ui_element)
            return

        # Let pygame_gui process input events first — if consumed, skip game input
