...
def test_exploration_scout_power_sums_int_and_agi():
    ...
    assert result["active_exploration"] is None
...
def test_exploration_chance_clamped_to_bounds():
    ...
    result2, _ = _advance(state2)
    assert result2["active_exploration"] is None
...
def test_exploration_success_reveals_node():
    ...
    for seed_val in range(100):
        session = _load()
        session.executor._lua.globals()["math.randomseed"](seed_val)
        lua_state = session.executor._to_lua(state)
        result = session.executor._lua.globals()["advance_cycle"](lua_state)
        py_result = _to_python(result)
        if py_result["planet_region"]["nodes"][0]["discovered"]:
            discovered = True
            break
...
def test_exploration_missing_target_node_no_error():
    ...
    slimes = [_slime("s1", int_val=10, agi_val=10)]
    state = _state_with_exploration(slimes, node_id="nonexistent_node", slime_ids=["s1"])
    result, _ = _advance(state)
