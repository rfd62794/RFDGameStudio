...
contract, confirmed by reading studio_mcp/publishing.py and
studio_mcp/game_metadata.py.
...
studio contract:
  ...
  - The entry at that key must itself be a JSON object (the post-publish
    hook studio_mcp.publishing.mark_itch_published sets
    `data[game_id]["pipeline_stage"]` after a real, successful butler push —
    that assignment requires the entry to already be a mapping).
  - The entry must carry a "pipeline_stage" field, one of the three real
    ...
    real, pipeline-tracked entry, even though the write-back
    would not itself crash on one (dict assignment creates missing keys).

Note: the itch.io push does NOT read game-metadata.json to decide *how* to
deploy (build_dir/itchio_slug/channel come only from publishing/games.yaml).
game-metadata.json is write-back-only: the studio's post-publish hook updates
it after a confirmed successful push, purely for pipeline-stage tracking. This
validator checks the entry is in the shape that write-back requires and
expects.
"""
...
def validate_publish_metadata(game_id: str, metadata_path: Path | None = None) -> None:
    ...
    contract the itch.io publish write-back requires.
