...
function assignRoles(agents: Agent[], possession: 'player' | 'opponent'): void {
  let carrierSet = false;
  for (const ag of agents) {
    if (ag.status !== 'active') {
      ag.role = 'inactive';
    } else if (ag.team === possession) {
      if (!carrierSet && ag.hasBall) {
        ag.role = 'carrier';
        carrierSet = true;
      } else {
        ag.role = 'escort';
...
function getCarrier(agents: Agent[]): Agent | null {
  for (const ag of agents) {
    if (ag.hasBall && ag.status === 'active') return ag;
  }
  return null;
}
