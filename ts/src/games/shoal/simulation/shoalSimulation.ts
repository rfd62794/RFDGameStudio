...
function spawnFish(st:ShoalState,x:number,depth:number):Fish{const f=newFish(st,x,depth);st.fish.push(f);st.stats.fish_count++;return f;}
function spawnShark(st:ShoalState,x:number,depth:number):Shark{const s=newShark(st,x,depth);s.lastMealTick=st.tickCount;st.sharks.push(s);st.stats.shark_count++;return s;}
...
export { initGameState, buildRenderState, tickGameInternal, countAlive, CONFIG };
