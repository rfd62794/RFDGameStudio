// Shoal â€” TS-Native Production Simulation
//
// Faithful port of Shoal's simulation logic from Lua (steering/logic/
// entities/state/utils/movement.lua + data.yaml), preserving the exact
// post-optimization algorithm including integer bucket keys/spatial
// hash/world-wrap/LCG/hysteresis/limit-turn/drag/depth-clamp/
// hunger-cold-exposure.
//
// This module replaces the fengari Lua executor call path for Shoal's
// tick_game. The Lua source files remain in games/shoal/*.lua as
// reference, per studio precedent (CorpWorld, KingMaker Squads).
//
// Recovered from the synthetic benchmark (commit 65403fd^, deleted as
// temporary), hardened for production: added handle_input, build_render_state,
...
const INHERIT_DRIFT_RANGE=15;
function extractHueFromHex(hex:string):number{const r=parseInt(hex.substr(1,2),16)/255,g=parseInt(hex.substr(3,2),16)/255,b=parseInt(hex.substr(5,2),16)/255;const max=Math.max(r,g,b),min=Math.min(r,g,b);if(max===min)return 0;const d=max-min;let h:number;switch(max){case r:h=((g-b)/d+(g<b?6:0))*60;break;case g:h=((b-r)/d+2)*60;break;default:h=((r-g)/d+4)*60;break;}return h;}
function generateInheritedColor(id:string,parentColor:string,liveColors:string[]):string{
  const match=id.match(/_(\d+)$/);const numericId=match?parseInt(match[1]):0;const hash=hashNumeric(numericId);
  const parentHue=extractHueFromHex(parentColor);
  const driftPrng=makePrng(hash);const drift=(driftPrng()-0.5)*2*INHERIT_DRIFT_RANGE;
  const hue=((parentHue+drift)%360+360)%360;
  const jitterHash=Math.floor(hash/1000)%1000;const jitterT=jitterHash/1000;
  const saturation=0.5+0.3*jitterT;const lightness=0.45+0.25*(1-jitterT);
  for(let attempt=0;attempt<=8;attempt++){const tryHue=(hue+attempt*40)%360;const[r,g,b]=hslToRgb(tryHue,saturation,lightness);if(!isTooClose(r,g,b)&&!isTooCloseToLive(r,g,b,liveColors))return rgbToHex(r,g,b);}
  for(let attempt=1;attempt<=360;attempt++){const tryHue=(hue+attempt)%360;const[r,g,b]=hslToRgb(tryHue,saturation,lightness);if(!isTooClose(r,g,b)&&!isTooCloseToLive(r,g,b,liveColors))return rgbToHex(r,g,b);}
  return '#808080';
...
// â”€â”€ Spatial hash â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Limit turn â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
interface ShoalState { world:typeof CONFIG.world; fish:Fish[]; sharks:Shark[]; algae:AlgaeCore[]; chunks:Chunk[]; nextId:number; tickCount:number; discreteAccum:number; prng:()=>number; spatialHash:SpatialHash; stats:Stats; resolvedSeed:number; events:unknown[]; }
let nextIdCounter=0;
// When true, new entities are created without FSMs (fallback to flat
// weighted-sum path). Used by the fixed-state equivalence test.
let _fsmDisabled = false;
export function _setFsmDisabled(v: boolean): void { _fsmDisabled = v; }
...
function spawnFleshChunks(st:ShoalState,x:number,depth:number,count:number):void{for(let i=0;i<count;i++){const angle=prngFloat(st.prng,0,Math.PI*2),speed=prngFloat(st.prng,20,60);st.chunks.push({id:uid('chunk'),x:x+Math.cos(angle)*prngFloat(st.prng,0,15),depth:depth+Math.sin(angle)*prngFloat(st.prng,0,15),vx:Math.cos(angle)*speed,vd:Math.sin(angle)*speed,radius:CONFIG.flesh_chunk.radius,floorTimer:0});}st.stats.chunk_count=st.chunks.length;}
function killCreature(st:ShoalState,creature:Fish|Shark):void{if(!creature.alive)return;creature.alive=false;if(creature.id.startsWith('fish'))st.stats.fish_count--;else st.stats.shark_count--;const minSpawn=CONFIG.flesh_chunk.min_spawn,maxSpawn=CONFIG.flesh_chunk.max_spawn;const count=Math.floor(st.prng()*(maxSpawn-minSpawn+1))+minSpawn;spawnFleshChunks(st,creature.x,creature.depth,count);}

// â”€â”€ Input handling (production addition â€” not in benchmark) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

function handleInput(st:ShoalState, input: InputState | null): void {
  if (!input) return;
  const tool = input.tool;
  if (!tool || !input.clicked) return;
  if (input.x === undefined || input.y === undefined) return;
  if (tool === 'cull') {
    cullAt(st, input.x, input.y, 40);
  } else if (tool === 'fish') {
    spawnFish(st, input.x, input.y);
  } else if (tool === 'shark') {
    spawnShark(st, input.x, input.y);
  } else if (tool === 'algae') {
    spawnAlgaeCore(st, input.x, input.y);
  }
}

function cullAt(st:ShoalState, x:number, depth:number, radius:number):void {
  for (const list of [st.fish, st.sharks]) {
    for (const c of list) {
      if (c.alive && distance(c.x, c.depth, x, depth) <= radius + c.radius) {
        killCreature(st, c);
      }
    }
  }
  for (const core of st.algae) {
    for (const n of core.nodules) {
      if (n.live && distance(n.x, n.depth, x, depth) <= radius + CONFIG.algae.nodule_radius) {
        grazeNodule(st, n);
      }
    }
  }
...
function grazeNodule(_st:ShoalState,nodule:Nodule):boolean{if(!nodule.live)return false;nodule.live=false;nodule.cooldown=CONFIG.algae.regrow_cooldown;return true;}
...
function tickGameInternal(st:ShoalState,dt:number):void{if(dt>0.1)dt=0.1;st.tickCount++;st.events=[];handleInput(st,null);rebuildSpatialHash(st);updateAlgae(st,dt);updateCreatures(st,dt);updateChunks(st,dt);updateDiscreteEvents(st,dt);st.stats.fish_count=countAlive(st.fish);st.stats.shark_count=countAlive(st.sharks);st.stats.algae_count=countAlgaeNodules(st);st.stats.chunk_count=st.chunks.length;}
...
function initGameState(seed:number,initialFish:number,initialSharks:number,initialAlgaeHubs:number):ShoalState{
  nextIdCounter=0;wanderTargets.clear();const prng=makePrng(seed);currentPrng=prng;
  const st:ShoalState={world:CONFIG.world,fish:[],sharks:[],algae:[],chunks:[],nextId:0,tickCount:0,discreteAccum:0,prng,spatialHash:{fish:new Map(),shark:new Map(),algae:new Map()},stats:{fish_count:0,shark_count:0,algae_count:0,chunk_count:0,seed},resolvedSeed:seed,events:[]};
  ...
  st.stats.fish_count=countAlive(st.fish);st.stats.shark_count=countAlive(st.sharks);st.stats.algae_count=countAlgaeNodules(st);
  return st;
}

// â”€â”€ Build render state (production addition â€” matches Lua build_render_state) â”€â”€

function buildRenderState(st:ShoalState):RenderState{
  const fish:ShoalCreature[]=st.fish.filter(f=>f.alive).map(f=>({id:f.id,x:f.x,depth:f.depth,radius:f.radius,color:f.lineageColor,angle:Math.atan2(f.vd,f.vx),mature:f.mature,hunger:f.hunger,cold_exposure:f.coldExposure,cold_damage:f.coldDamage}));
  const sharks:ShoalCreature[]=st.sharks.filter(s=>s.alive).map(s=>({id:s.id,x:s.x,depth:s.depth,radius:s.radius,color:s.lineageColor,angle:Math.atan2(s.vd,s.vx),mature:s.mature,hunger:s.hunger,cold_exposure:s.exposure}));
  const algae:RAlgaeCore[]=st.algae.map(core=>({id:core.id,x:core.x,depth:core.depth,nodules:core.nodules.filter(n=>n.live).map(n=>({x:n.x,depth:n.depth,radius:CONFIG.algae.nodule_radius}))}));
  const chunks:FleshChunk[]=st.chunks.map(c=>({x:c.x,depth:c.depth,radius:c.radius,decay_ratio:c.floorTimer?Math.min(1,c.floorTimer/CONFIG.flesh_chunk.floor_grace_time):0}));
  return{world:{width:st.world.width,height:st.world.height},fish,sharks,algae,chunks,stats:st.stats,events:st.events,tick_count:st.tickCount};
}
...
export function createShoalSimulation(): ShoalSimulation {
  ...
  return {
    ...
    tickGame(dt: number, input: InputState | null): RenderState {
      if (!st) throw new Error('call initGame first');
      if (dt > 0.1) dt = 0.1;
      st.tickCount++;
      st.events = [];
      ...
      updateAlgae(st, dt);
      updateCreatures(st, dt);
      updateChunks(st, dt);
      updateDiscreteEvents(st, dt);
      st.stats.fish_count = countAlive(st.fish);
      st.stats.shark_count = countAlive(st.sharks);
      st.stats.algae_count = countAlgaeNodules(st);
      st.stats.chunk_count = st.chunks.length;
      return buildRenderState(st);
    },
    getState(): ShoalState | null {
      return st;
...
export { initGameState, buildRenderState, tickGameInternal, countAlive, CONFIG, generateInheritedColor, generateProceduralColor };
export type { ShoalState, Fish, Shark, Nodule, AlgaeCore as AlgaeCoreInternal, Chunk };
