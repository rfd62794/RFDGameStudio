...
export default function App({ session }: GameRendererProps) {
  ...
  const handlePlayCard = useCallback((card: DeckCard) => {
    ...
    if (result.fightWon === true) {
      ...
      const slots = call(
        ...
        tier,
        data,
        result.nextState.nextRewardBias ?? null
      ) as RewardSlot[] | null;
  ...
  const handleTreasureChoice = useCallback(
    (choice: 'essence' | 'relic') => {
      if (!run) return;
      const next = call('resolve_treasure', run, choice, data) as RunState | null;
      if (!next) return;
      const advanced = call('advance_node', next) as RunState | null;
      if (advanced) setRun(advanced);
    },
    [run, call, data]
  );

  const handleStorePurchase = useCallback(
    (index: number) => {
      if (!run) return;
      // Lua tables are 1-indexed; the UI passes a 0-based slot index.
      const next = call('resolve_store', run, index + 1, data) as RunState | null;
      if (!next) return;
      const advanced = call('advance_node', next) as RunState | null;
      if (advanced) setRun(advanced);
    },
  ...
  const handleStoreLeave = useCallback(() => {
    if (!run) return;
    const next = call('resolve_store', run, null, data) as RunState | null;
    if (!next) return;
    const advanced = call('advance_node', next) as RunState | null;
    if (advanced) setRun(advanced);
  }, [run, call, data]);

  const handleAnomalyAccept = useCallback(() => {
    if (!run) return;
    const next = call('resolve_anomaly', run, data) as RunState | null;
    if (!next) return;
    const advanced = call('advance_node', next) as RunState | null;
    if (advanced) setRun(advanced);
  }, [run, call, data]);
    ...
    <GameShell gameLabel="Dissonance Depths" gameId="dissonance" phase="Renderer Phase A" statusArea={statusArea}>
      <div className="h-full overflow-y-auto bg-slate-950 p-4">
        ...
        {appPhase === 'run' && run && run.status === 'combat' && !rewardSlots && (
          <CombatPhase run={run} onPlayCard={handlePlayCard} data={data} />
        )}
        ...
        {appPhase === 'run' && run && run.status === 'treasure' && (
          <TreasurePhase run={run} onChoice={handleTreasureChoice} onLeave={() => handleTreasureChoice('essence')} />
        )}

        {appPhase === 'run' && run && run.status === 'store' && (
          <StorePhase run={run} onPurchase={handleStorePurchase} onLeave={handleStoreLeave} />
        )}

        {appPhase === 'run' && run && run.status === 'anomaly' && (
          <AnomalyPhase run={run} onAccept={handleAnomalyAccept} />
        )}
