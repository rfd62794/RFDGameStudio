  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-8">
      <header className="mb-6">
        <h1 className="text-3xl font-bold">{game?.name ?? 'Dissonance Depths'}</h1>
      ...
      <section className="max-w-2xl">
        ...
        <div className="rounded-lg bg-slate-800 p-4 font-mono text-sm text-slate-300">
          <p>Game ID: {game?.id as string}</p>
          <p>Version: {game?.version as string}</p>
          <p>Studio: {game?.studio as string}</p>
        </div>
      </section>
    </div>
  );
