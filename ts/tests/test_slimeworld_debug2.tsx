...
import { stateToLua, luaNodeToTs, type LabState, type Slime, type SlimeColor } from '../src/games/slimeworld/types';
...
function makeMinimalState(): LabState {
  ...
  return {
    ...
    cultureRelationships: { Red: 100 } as Record<SlimeColor, number>,
    recentMarketSales: [], petitions: [],
  };
}

describe('Debug Fealty', () => {
  it('debug_fealty', () => {
    const state = makeMinimalState();
    const luaState = stateToLua(state);
    console.log('lua culture_relationships:', JSON.stringify(luaState['culture_relationships']));
    const [raw] = call(session, 'advance_cycle', luaState);
    const result = raw as Record<string, unknown>;
    console.log('result culture_relationships:', JSON.stringify(result['culture_relationships']));
    const region = result['planet_region'] as Record<string, unknown>;
    const nodes = region['nodes'] as Array<Record<string, unknown>>;
    for (const n of nodes) {
      console.log('node:', n['id'], 'owner:', n['owner_color'], 'fealty:', n['fealty_locked'], 'pressure:', JSON.stringify(n['pressure']));
    }
